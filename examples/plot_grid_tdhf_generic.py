...
example:    plotting simulation results from grid-tdhf using db-tools.
            this example uses the generic program agnostic DBTools class.
            there is also a specific backend implementaion for grid-tdhf,
            and this can be made for any other program as well.
            see e.g. plot_grid_tdhf_minimal.py or
            plot_grid_tdhf_multiple_runs.py for usage examples
"""
...
# optional: base filters which will be included in
# every subsequent .search() call unless you override it
output_dir = "output"

dbtools = (
    DBTools()
    .with_prefix(output_dir)
    .with_base_filters(
        tag="example_runs",
        E0=0.01,
        dt=0.1,
...
fileroot1 = dbtools.search(r_max=32, N=48).fileroots[0]
fileroot2 = dbtools.search(r_max=36, N=54).fileroots[0]
fileroot3 = dbtools.search(r_max=40, N=60).fileroots[0]
